...
class PostPagesTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.comment = Comment.objects.create(
            post=cls.post, author=cls.author, text="Тестовый комментарий"
        )
        cls.name_templates = (
        ...
        for name, template, arg in self.name_templates:
            if name is names:
                with self.subTest(template=template):
                    ...
                    first_object = response.context["page_obj"][COUNT_POST_NULL]
                    check_post_2(first_object, self.post)
                    check_group(first_object.group, self.group)
                    self.assertNotEqual(first_object.group.title, self.group_not.title)
                    self.assertEqual(first_object.image, f"posts/{self.uploaded.name}")

    def test_post_detail_page_show_correct_context(self):
        """Шаблон post_detail сформирован с правильным контекстом."""
        response = self.authorized_author.get(self.post_detail)
    ...
    def test_cache_index_page(self):
        ...
        post = Post.objects.create(author=self.author, text="Тестируем кеш")

        cache.clear()
        content_index = self.authorized_author.get(self.index).content
        Post.objects.filter(author=post.author, text=post.text).delete()
        self.assertEqual(self.authorized_author.get(self.index).content, content_index)
        cache.clear()
        self.assertNotEqual(
            self.authorized_author.get(self.index).content, content_index
        )
    ...
    def test_follow_authorized_user(self):
        ...
        self.authorized_user.post(
            "/profile/add/follow/", data={"author": self.post.author.id}
        )
        self.assertEqual(self.user.follower.count(), count_follow + 1)
        self.authorized_user.post(
            "/profile/add/follow/", data={"author": self.post.author.id}
        )
        self.assertEqual(self.user.follower.count(), count_follow)
    ...
    def test_follow_index_count(self):
        ...
        authorized_user_2.force_login(User.objects.create_user(username="User_2"))

        self.authorized_user.post(
            "/profile/add/follow/", data={"author": self.post.author.id}
        )
        count_user = len(
            self.authorized_user.get(self.follow_index).context["page_obj"]
        )
        count_user_2 = len(authorized_user_2.get(self.follow_index).context["page_obj"])
        Post.objects.create(author=author, text=text)
        self.assertEqual(
            len(self.authorized_user.get(self.follow_index).context["page_obj"]),
            count_user + 1,
        )
        self.assertEqual(
            len(authorized_user_2.get(self.follow_index).context["page_obj"]),
            count_user_2,
        )
...
class PaginatorViewsTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        for i in range(1, 14):
            cls.post = Post.objects.create(
                author=cls.user, text=f"Тестовый пост {str(i)}", group=cls.group
            )
        cls.index = "/"
    ...
    def test_page_contains_records(self):
        ...
        response = self.authorized_client.get(self.index + "?page=2")
