...
class TestCreateView:
    ...
    def test_create_view_post(self, mock_media, user_client, user, group):
        ...
        assert response.status_code in (301, 302), (
            ...
            f"перенаправляете на страницу профиля автора `posts/profile/{user.username}`"
        )
        ...
        assert (
            response.url == f"posts/profile/{user.username}/"
        ), f"Проверьте, что перенаправляете на страницу профиля автора `posts/profile/{user.username}`"
